#! /usr/bin/env python
# -*- coding:utf-8-*-
import logging, weather, funcs, yt
...
def menu(bot, update):
    ...
    keyboard = [[InlineKeyboardButton("Tiempo", callback_data='1'),
                 InlineKeyboardButton("Descarga", callback_data='2')]]
...
def handle(bot, update, choice, query):
    if (choice == '1'):
    	    ...
    	    bot.edit_message_text(text="Modo de descarga activado",
			  chat_id=query.message.chat_id,
			  message_id=query.message.message_id)
	    yt.download(bot,update,query.message.chat_id) 
...
def main():
    # TOKEN
    updater = Updater('357116280:AAEpGcgOng5u8betuNUj8c865eFqGX49JGo')
